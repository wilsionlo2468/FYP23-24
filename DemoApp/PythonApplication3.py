...
from amplify_color import VidMag
...
class GUI(QMainWindow, QThread):
    ...
    def main_loop(self):
        frame1 = self.input.get_frame()
        # frame1 = self.webcam.get_frame()
        f1 = imutils.resize(frame1, width = 256)
        self.amplify_color.data_buffer.append(f1)
        self.amplify_color.run_color()
        frame = imutils.resize(cv2.convertScaleAbs(self.amplify_color.frame_out), width = 640)
        #frame=cv2.convertScaleAbs(self.amplify_color.frame_out)
